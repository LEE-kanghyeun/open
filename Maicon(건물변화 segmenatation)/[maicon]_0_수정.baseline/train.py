...
if __name__ == '__main__':
    ...
    train_dirs = os.path.join(data_dir,'train', 'x')
    # train_dirs = os.path.join(prj_dir, 'data', 'train','x')
    # print("################# train_dirs",train_dirs)
    # Load data and create dataset for train 
    # Load image scaler
    #------------------------------------------------------ 
    # Path 고정
    #train_img_paths = glob(os.path.join("/mnt/e/01.Eric/02.Source/09.[Maicon]_Change_Dtection/0.baseline/data/train", 'x', '*.png'))
    train_img_paths = [os.path.join(train_dirs,i) for i in os.listdir(train_dirs) if i.endswith(".png")]
    ...
    print('model is ' , config['architecture'], '\n')
    print('encoder is ', config['encoder'], '\n')
    print('encoder weight is ', config['encoder_weight'], '\n')
    ...
    print('lossfunc is ', config['loss']['name'], '\n')
