...
class PolicyNetwork(nn.Module):
    ...
    def select_action(self, state):
        with torch.no_grad():
            prob = self.forward(state)  # prob : 각 action을 실행할 확률  ex) [ 0.82, 0.18 ]
            b = Bernoulli(prob)  # prob 값을 기반으로 베르누이 확률 분포로 만듬
            action = b.sample()  # b(베르누이) 확률 분포에서 임의로 action을 샘플링
        return action.item()
...
class Memory(object):
    def __init__(self, memory_size: int) -> None:
        ...
        self.buffer = deque(maxlen=self.memory_size) # self.memory_size 의 크기만큼만 채우고, 가득찬 이후로는 새로운 데이터 입력시 오래된 데이터 삭제함
    ...
    def sample(self, batch_size: int, continuous: bool = True):
        ...
        if continuous:
            rand = random.randint(0, len(self.buffer) - batch_size)              # 랜덤 샘플링으로 배치 데이터 수집
            return [self.buffer[i] for i in range(rand, rand + batch_size)]      # 랜덤 샘플링으로 배치 데이터 수집
        else:
