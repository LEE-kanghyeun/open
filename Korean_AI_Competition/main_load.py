...
if __name__ == '__main__':
    ...
    args.add_argument('--batch_size', type=int, default=64) # vgg 돌릴 경우 cuda 오류로 64->32로 변경함 2022.09.08 06:00
    ...
    if config.mode == 'train':
        #현재까지 best score
        #nsml.load(checkpoint='49',session='KAIC311/t2-conf-final/2')

        #현재까지 best Score 9/7 14:00
        #nsml.load(checkpoint='17',session='KAIC311/t2-conf-final/9')

        #이걸로추가학습하긴 해야함 -> cnn 모델
        # nsml.load(checkpoint='18',session='KAIC311/t2-conf-final/14')

        #vggextractor 적용 33번 에폭 -> 50번까지는 가봐야!!
        #nsml.load(checkpoint='33',session='KAIC311/t2-conf-final/34')
        ...
        # best score
        # nsml.load(checkpoint='46',session='KAIC311/t2-conf-final/2')
        # nsml.save("0")
        # exit()

        #best score 불러와서 인퍼런스 시켜봄
        nsml.load(checkpoint='17',session='KAIC311/t2-conf-final/14')
        nsml.save("0")
        exit()
