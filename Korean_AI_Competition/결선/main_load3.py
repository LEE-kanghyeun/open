...
def result_collect(model, path_list, return_list):
    print('1')
    # print(len(path_list))
    for path in path_list:
        return_list.append(
            {
        'filename' : path.split('\\')[-1],
        'text' : single_infer(model, path)[0]
            }
        )
...
def inference(path, model, **kwargs):
    ...
    for i in range(n):
        p = Process(target=result_collect, args=(model, path_split_list[i],return_list))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()
...
if __name__ == '__main__':
    ...
    args.add_argument('--batch_size', type=int, default=32) # vgg 돌릴 경우 cuda 오류로 64->32로 변경함 2022.09.08 06:00
    args.add_argument('--save_result_every', type=int, default=10)
    args.add_argument('--checkpoint_every', type=int, default=1)
    args.add_argument('--print_every', type=int, default=10)
    args.add_argument('--dataset', type=str, default='kspon')
    args.add_argument('--output_unit', type=str, default='character')
    # args.add_argument('--num_workers', type=int, default=8) # 8-> 4
    # args.add_argument('--num_threads', type=int, default=16) # 16 -> 8
    args.add_argument('--num_workers', type=int, default=16) # 8-> 4
    args.add_argument('--num_threads', type=int, default=64) # 16 -> 8
    ...
    # args.add_argument('--final_lr', type=float, default=1e-04)
    # args.add_argument('--peak_lr', type=float, default=1e-01)
    ...
    args.add_argument('--architecture', type=str, default='las')
    ...
    if config.mode == 'train':
        ...
        nsml.load(checkpoint='6',session='KAIC311/t2-conf-final/98')
        nsml.save("986")
        exit()
        ...
        for epoch in range(num_epochs):
            ...
            if best_model:
                print('[INFO] BEST Epoch %d (Validation) Loss %0.4f  CER %0.4f' % (epoch, valid_loss, valid_cer))
